...
export const singleChat = asyncHandler(async (req, res) => {
  try {
    ...
    const { thinking, output, analysis } = extractOutput(
      response.content[0].text
    );
    return res.status(200).send({
      id: 1,
      data: {
        thought_process: thinking[0],
        output: output[0],
        analysis: analysis[0],
      },
    });
...
export const conflictChat = asyncHandler(async (req, res) => {
  try {
    const { uuid, question } = req.authData;
    let chat = "",
      isConvo = false;
    ...
    const system_prompt = `${conversation_system_prompt_v2}
    IMPORTANT VERIFICATION REQUIREMENTS:
    - You MUST maintain this exact language in your response
    - If input is in Hinglish, your output MUST be in Hinglish
    - You MUST preserve the full conversation with the exact number of exchanges
    ...
    The input format is: ${isConvo ? "A multi-person conversation, hence the provide the emotional state of each person" : "A single message, hence provide the emotional state of the person"}`;
    ...
    const cost = calculateCost(response.usage);
    logger.info(`Cost of the request: Rs.${cost}`);
    const { thinking, output, analysis, emotional_state } = extractOutput(
      response.content[0].text
    );
    return res.status(200).send({
      id: 1,
      data: {
        ...
        thought_process: thinking[0],
        analysis: analysis[0],
        emotional_state: emotional_state[0],
      },
    });
  } catch (error) {
    logger.error(error.message);
